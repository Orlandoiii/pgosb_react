...
export default function TableDataGridV2({ }) {

    logger.log("DATA:", moock_data)


    const COLUMNS = [];

    Object.entries(moock_data[0])
        .forEach(([key, _]) => {
            COLUMNS.push(
                {
                    header: key,
                    ...
                    <table className="border-collapse w-full mt-2" >
                            ...
                            {table.getHeaderGroups().map(headerGroup =>
                                <tr className="sticky top-0" key={headerGroup.id}>
                                    {headerGroup.headers.map(header =>
                                        <th className={`border-y bg-white 
                                        ...
                                        text-[#64748b] flex justify-center items-center">
                                                    ...
                                                    <SortIcon isSorted={header.column.getIsSorted}
                                                        isSortedDesc={table.getState().sorting.find(sortItem => sortItem.id === header.column.id)?.desc} />
                                                </div>
                        ...
                        <tbody>
                            {table.getRowModel().rows.map(row =>
                                <tr key={row.id} className=" text-[#64748b] overflow-auto">
                                    {row.getVisibleCells().map(cell =>
                                        <td key={cell.key} className="whitespace-nowrap first:text-[#3c50e0] h-20 p-2 text-sm text-center font-medium border-b border-gray-200">
