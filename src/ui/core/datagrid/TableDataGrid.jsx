...
function BackwardButton({ onClick, disabled = false }) {
    return (
        <button
            onClick={onClick}
            className={`flex cursor-pointer items-center justify-center rounded-md p-1 px-2 hover:bg-btc_primary text-btc_primary hover:text-white stroke-btc_primary hover:stroke-white
...
function ForwardButton({ onClick, disabled = false }) {
    return (
        <button
            onClick={onClick}
            disabled={disabled}
            className={`flex cursor-pointer items-center justify-center rounded-md text-btc_primary stroke-btc_primary hover:stroke-white
            p-1 px-2 hover:bg-btc_primary hover:text-white  ${disabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}
        >
            <svg
                className="fill-current "
                width="28"
...
function NumberButton({ number = 1, active = false, onClick }) {
    return (
        <button
            onClick={onClick}
            className={` false h-8 mx-1 flex cursor-pointer items-center justify-center rounded-md  font-semibold
            p-1 px-3 ${active ? 'bg-btc_primary text-white' : ' text-btc_primary'} 
        hover:bg-btc_secondary hover:text-white`}
        >
            {number}
        </button>
    )
    ...
    return (
        ...
        >
            <input
                ...
                className=" border-stroke bg-white text-gray-700  focus:border-white px-3 py-1  rounded-md w-full h-auto font-light text-sm outline-none"
            />
        </span>
    )
}
...
function DateColumnFilter({ column }) {
    ...
    return (
        <>
            <button className=' bg-white text-gray-700  focus:border-white px-0.5 py-0.5  rounded-md w-full h-auto font-light text-sm outline-none' type='button' onClick={(e) => {
                e.stopPropagation()
                setOpen(true)
            }}>
                ...
                <div className="flex space-x-2 pl-6 pr-1 py-1">
                    {showEditButton && permissions['update'] && (
                        <button
                            ...
                            className="flex justify-center items-center bg-btc_primary text-white rounded-full w-9 h-9 shadow-md opacity-50 hover:opacity-100"
                        >
                    ...
                    {showDeleteButton && permissions['delete'] && (
                        <button
                            ...
                            className="flex justify-center items-center text-white rounded-full w-9 h-9 bg-btc_red shadow-md opacity-50 hover:opacity-100"
                        >
        ...
        <>
            <div className='flex flex-col pb-24 w-full h-full'>
                <div className="flex flex-col flex-1 bg-[white] h-60 overflow-hidden">
                    <header className="flex justify-between mx-auto px-8 py-4 w-full  bg-btc_secondary rounded-t-md">
                        ...
                        <table className="border-collapse w-full">
                            <thead>
                                ...
                                {table.getHeaderGroups().map((headerGroup) => (
                                    <tr
                                        className="top-0 sticky bg-btc_gray z-40"
                                        key={headerGroup.id}
                                    >
                                        {headerGroup.headers.map((header) => (
                                            <th
                                                key={header.column?.id}
                                                className={` bg-btc_secondary
                                                      h-20 px-1 cursor-pointer 
                                             ${header.column.getCanSort() ? 'cursor-pointer' : 'cursor-none'}
                                             ${header.column.id === 'select' ? 'sticky left-0 z-50' : ''}
                                            `}
                                                style={{
                                                    left: header.column.id === 'select' ? 0 : undefined, // Ensure solid background color
                                                }}
                                                {...{
                                                    onClick:
                            ...
                            <tbody>
                                {table.getRowModel().rows.map((row, rowIndex) => (
                                    <tr
                                        key={row.id}
                                        className={`${rowIndex % 2 === 0 ? 'bg-btc_tertiary bg-opacity-60' : 'bg-white'} text-[#0A2F4E] 
