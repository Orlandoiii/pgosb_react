...
export default function Toggle({ active, onClick }: ToggleProps) {
    return (
        <button
            type="button"
            className={`relative  w-[48px]  h-[25px]  ring-2   ${
                active
                    ? 'bg-[#0A2F4E] ring-offset-2 ring-[#0A2F4E]'
                    : 'bg-[#E5E7EB] ring-offset-1 ring-gray-400'
            }  flex px-1 py-1 rounded-full shadow-sm border-1 border-slate-400 
                transition-all ease-in-out duration-300`}
            onClick={onClick}
        >
            <span
                className={`absolute transition-position ease-in-out duration-300 
                              top-1/2 transform -translate-y-1/2 ${active ? 'translate-x-full opacity-100' : 'translate-x-0 opacity-0'}`}
            >
                <CheckIcon />
            </span>
            <span
                className={`absolute transition-position ease-in-out duration-300 right-1.5
