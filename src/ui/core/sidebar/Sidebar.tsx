...
export default function Sidebar({}) {
    ...
    function onClikcLogOut() {
        const respose = showConfirmationModal(
            'Cerrar Sesion',
            'Esta segur@ que desea salir del sistema'
        )
    ...
    return (
        <>
            <aside
                className={`relative h-screen min-w-[90px]   
        bg-btc_primary shadow-xl  overflow-hidden  flex flex-col
         ...
         rounded-r-3xl ${collapse ? '' : ' md:min-w-[300px]'}`}
            >
                <div className={`relative  py-6 ${collapse ? '' : 'md:px-10'}`}>
                    <div
                        className={`w-full flex justify-center  items-center  ${collapse ? '' : ' md:justify-start md:space-x-2'}`}
                    >
                        ...
                        <h1
                            className={`absolute text-[whitesmoke] 
                        ...
                        <button
                            className={`absolute ${collapse ? 'opacity-0' : ' md:top-1/2  md:right-1 md:transform md:-translate-y-1/2'}  
                        ...
                        >
                            <div
                                className={`${collapse ? 'rotate-180 right-8' : 'rotate-0 right-1'}`}
                            >
                                <BiArrowFromRight
                                    color="whitesmoke"
                                    size={30}
                ...
                <div className="flex flex-col overflow-y-auto no-scrollbar">
                    <nav className="w-full mt-8 px-4">
                        <div className="space-y-3">
                            <h2 className="px-2 uppercase text-[whitesmoke] text-center md:text-left opacity-60  text-md mb-5 font-semibold">
                                Menu
                            </h2>
                ...
                <button
                    className="absolute w-full   px-6 top-[90%]"
                    onClick={() => {
                        onClikcLogOut()
                    }}
                >
                    <div
                        className={`relative w-full py-3 flex justify-center   items-center space-x-4 hover:bg-btc_secondary 
                hover:text-white rounded-xl  px-2 ${collapse ? '' : 'md:justify-start'}`}
                    >
                        ...
                        opacity-0 text-white text-xl ${collapse ? '' : ' md:left-10 md:opacity-100 '}`}
                        >
                            Cerrar Sesion
                        </p>
                    </div>
