...
function KeyValue({ keyName, value }) {
    ...
    return (
        ...
        <div className="w-full flex justify-between items-baseline">
            ...
            <p className="capitalize-first text-md">{keyName}:</p>

            {valueIsArray ?
                <ArrayElement title={keyName} values={value} /> :
                <p className="text-[blue] text-sm">{isBoolean(value) ? value.toString() : value}</p>}
...
export default function Accordion({ title, value }) {
    ...
    return (
        <div className="md:min-w-[600px] h-auto rounded-md border border-slate-200 p-4 shadow-lg md:p-6 xl:p-7.5 bg-gray-50">
            <button className="flex w-full items-center justify-between  gap-2  py-2 px-2 bg-[#161D2A] rounded-md shadow-md" onClick={() => { setOpen(o => !o) }}>
                <div className="">
                    <h4 className="text-left text-title-xsm font-bold text-[whitesmoke] uppercase">{title}</h4>
                </div>
                <div className="flex h-9 w-full max-w-9 items-center justify-center rounded-full border-2 border-[whitesmoke] dark:border-white">
                    <svg className={`fill-[whitesmoke] transition-all duration-200 ease-in-out w-[15px] h-[15px] ${open ? "hidden" : "block"}`} viewBox="0 0 15 15" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <svg className={`fill-[whitesmoke] transition-all duration-200 ease-in-out w-[15px] h-[15px] ${open ? "block" : "hidden"}`} viewBox="0 0 15 3" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={`mt-5 transition-all duration-700 ease-in-out 
                ...
                <div className="flex-1 ">
                    {firstHalf && Object.entries(firstHalf).map(([idx, value]) => {
                        return <KeyValue keyName={value[0]} value={value[1]} key={value[0]} />
                    })}
                </div>
