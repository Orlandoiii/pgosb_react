...
async function LoadLayout(endpoint) {
    ...
    while (retryCounter < 5) {
        logger.info(
            `Haciendo Request para cargar layout intento:${retryCounter} ${endpoint}`
        )
        ...
        try {
            ...
            if (layoutRequest.status < 200 || layoutRequest.status > 299) {
                logger.error(
                    'El resultado del request para buscar layout no fue exitoso'
                )
    ...
    return {
        errList: errList,
        err: 'El numero de reintento se agoto, para la busqueda del layout',
        success: false,
    }
}
...
const endpoints = [
    ...
    { endpoint: '/api/v1/layout/service', layout: 'service_layout' },
    { endpoint: '/api/v1/layout/mission', layout: 'mission_layout' },
]
