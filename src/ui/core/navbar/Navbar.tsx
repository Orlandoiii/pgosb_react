        ...
        if (password.length > 0 || newPassword.length > 0) {
            if (!passwordRegex.test(password)) {
                setPasswordErrMessage(
                    'Contraseña invalida, debe contener una mayuscula, un caracter especial y al menos 8 caracteres'
                )
                return
            } else {
                setPasswordErrMessage('')
...
export default function Navbar({}) {
    ...
    return (
        <>
            <nav
                className="sticky  top-0 flex w-full h-[80px] 
        bg-btc_tertiary shadow-sm rounded-sm  z-10"
            >
                <div
                ...
                >
                    <div className="pr-12">
                        <div className="flex items-center space-x-3">
                            ...
                            <div
                                className={`flex flex-col justify-center items-end transition-all ease-in-out duration-500 
                                ${userDataIsLoad ? ' opacity-100 translate-x-0' : ' opacity-0 -translate-x-10'}`}
                            >
                                <h3 className="text-lg font-semibold self-start text-rose-700">
                                    {name}
                                </h3>
