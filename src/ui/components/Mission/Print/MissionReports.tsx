...
export function MissionReports({
    ...
    const [selectedReport, setSelectedReport] = useState<string>(
        reportTypes.StatisticsForAntares
    )
    const reports: string[] = EnumToStringArray(reportTypes)

    return (
        <ModalLayout title={'Registro de Persona'} onClose={closeOverlay}>
                ...
                <div className="relative flex h-full w-full space-x-8">
                    <div className="flex h-full w-72 flex-col space-y-4 flex-none ">
                        <Chips
                            ...
                            onSelectionChanged={setSelectedReport}
                        />
                    </div>
                    ...
                    <div className="h-full w-full flex flex-col rounded-lg border border-slate-400 bg-slate-300 overflow-auto">
                        <PrintView>
                            ...
                            {selectedReport ===
                                reportTypes.StatisticsForAntaresTypes && (
                                <DetailServicesSummaryPrint
                                    missionsIds={servicesIds}
                            ...
                            {selectedReport ===
                                reportTypes.StatisticsForStation && (
                                <DetailServicesSummaryPrint
                                    missionsIds={servicesIds}
                                    groupBy={'Stations'}
                                    filters={filters}
