import { CreateElementFunction, OverlayItem, OverlayModalConfig } from "./models/overlay_item";
import Alert, { AlertTypes } from "../notifications/alert";
...
class ModalService {
  ...
  pushAlert(
    ...
    const newModal = new OverlayItem(
      Alert,
      { type: type, message: text, onResponse: onResponse, closeOverlay: undefined },
  ...
  closeModal = (id: string) => {
    this.modals = this.modals.map((modal) => (modal.id === id ? modal.withVisibility(false) : modal));
    this.updateModals();
  };

  removeModal = (id: string) => {
    console.log("ModalRemove", id);
  ...
  closeToast = (id: string) => {
    this.toasts = this.toasts.map((toast) => (toast.id === id ? toast.withVisibility(false) : toast));
    this.updateToasts();
  };
