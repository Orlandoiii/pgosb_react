export function isValidAccount(value: string, bankCode: string): string {
  ...
  if (value.length === 20 && startsWith(value, "0001") && !endsWith(value, bankCode)) {
...
function startsWithValidCode(value: string, bankCode: string, validPrefixes: string[]): boolean {
  return validPrefixes.some((prefix) => startsWith(value, prefix)) || startsWith(value, bankCode);
}
...
function getDigitValue(digits: string): number {
  ...
  let result = digits
    .split("")
    .reduce((sum, digit, index) => sum + parseInt(digit, 10) * weights[index % weights.length], 0);
